...
const CalendarioLaboral = () => {
    ...
    useEffect(() => {
        axios.get(`http://54.74.52.150:3030/eventosCalendario?empresa=${cookies.get('empresa')}&idTrabajador=0`).then((data) => {
            setEvents(data.data);
        });
    ...
    const handleOnChange = (e) => {
        axios.get(`http://54.74.52.150:3030/eventosCalendario?empresa=${cookies.get('empresa')}&idTrabajador=${e.target.value}`).then((data) => {
            setEvents(data.data);
        });
    }
    const updateEvents = () => {
